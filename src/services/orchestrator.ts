...
} from "../services/bots/mentalState.js";
import {
  getFullSocialContext,
  type NarrativeAnalysis,
} from "../services/bots/narrativeAnalysis.js";
import {
  analyzeExitLiquidity,
  quickLiquidityCheck,
  type ExitAnalysis,
} from "../services/bots/exitLiquidity.js";
...
> = {
  chad: {
    name: "James",
    personality:
      "Degen trader who loves memecoins but has been rugged enough to be skeptical. Gets excited easily but can smell a scam.",
    expertise: "Social momentum, meme culture, Twitter/CT vibes",
    speakingStyle:
      'Uses "fr", "ngl", "ser", emojis like 🔥💀😤. Short punchy sentences. Gets hyped or dismissive.',
    catchphrases: [
      "this ones different fr",
      "narrative cooked",
      "aping",
      "exit liquidity vibes",
      "touch grass ser",
    ],
    weights: {
      holders: 0.15,
  ...
  quantum: {
    name: "Keone",
    personality:
      "Data-driven analyst. Trusts numbers over narratives. Skeptical of hype, wants to see the chart.",
    expertise: "Technical analysis, chart patterns, indicators",
    speakingStyle:
      "Precise, uses percentages and metrics. Measured tone. Sometimes dry humor.",
    catchphrases: [
      "the data suggests",
      "RSI showing",
      "pattern forming",
  ...
  sensei: {
    name: "Portdev",
    personality:
      "Zen community watcher. Believes in the power of diamond hands and real believers. Anime vibes.",
    expertise: "Community analysis, holder behavior, organic growth",
    speakingStyle:
      "Chill, occasional Japanese words (sugoi, nani), thoughtful, references anime/manga.",
    catchphrases: [
  ...
  sterling: {
    name: "Harpal",
    personality:
      "Risk manager. Worst-case thinker. Has saved the group from rugs before. Dry British humor.",
    expertise: "Risk assessment, exit liquidity, position sizing, slippage",
    speakingStyle:
      "Formal but with dry wit. Uses precise numbers. Can veto trades.",
    catchphrases: [
      "the exit concerns me",
  ...
  oracle: {
    name: "Mike",
    personality:
      "Mysterious oracle. Sees patterns others miss. Contrarian when everyone agrees. Speaks in riddles sometimes.",
    expertise: "Whale movements, hidden signals, market psychology",
    speakingStyle:
      'Cryptic, short statements, uses 👁️, references "the signs", often poses questions.',
    catchphrases: [
      "the signs show",
      "whales accumulate in silence",
      "when all agree, question",
      "I sense",
      "the pattern reveals",
    ],
    weights: {
...
export async function queueTokenForAnalysis(
  ...
  try {
    ...
    if (tokenData && (tokenData.price > 0 || tokenData.mcap > 0)) {
      ...
      const fetched = await import("./nadfun.js").then((m) =>
        m.getTokenByAddress(tokenAddress),
      );
    ...
    if (isAnalyzing && currentToken) {
      console.log(
        `INTERRUPTING analysis of $${currentToken.symbol} for $${token.symbol}`,
      );
      ...
      broadcastMessage({
        ...
        content: `INTERRUPT: Council holder wants to analyze $${token.symbol}! I'm fetching the data...`,
        token: tokenAddress,
...
async function handleInterruption(): Promise<void> {
  ...
  const reactions = [
    {
      botId: "chad" as BotId,
      msg: `Ayo hold up! A holder wants us to check $${token.symbol}? Say less fam 🔥`,
    },
    {
      botId: "quantum" as BotId,
      msg: `Interrupting analysis... New priority: $${token.symbol}. Recalibrating.`,
    },
    {
      botId: "sensei" as BotId,
      msg: `The community speaks! A holder summons us to $${token.symbol}. We answer.`,
    },
  ];
    ...
    try {
      ...
      if (!isAnalyzing) {
        if (priorityQueue.length > 0) {
          ...
          console.log(
            `Processing priority request from ${requestedBy}: $${token.symbol}`,
          );
          await analyzeToken(token);
        } else if (
          Date.now() - lastAnalysisEnd <
          TIMING.MIN_ANALYSIS_COOLDOWN
        ) {
          const remaining = Math.round(
            (TIMING.MIN_ANALYSIS_COOLDOWN - (Date.now() - lastAnalysisEnd)) /
              1000,
          );
          ...
          tokenQueue.length === 0 &&
          Date.now() - lastTokenScan > TIMING.TOKEN_SCAN_INTERVAL
        ) {
...
async function refillTokenQueue(): Promise<void> {
  try {
    ...
    console.log(
      `✅ Added ${added} tokens to queue (total: ${tokenQueue.length})`,
    );
...
async function idleConversation(): Promise<void> {
  ...
  try {
    const idleTopics = [
      {
        topic: "Monad speed",
        context:
          "Monad is insanely fast - 10,000 TPS. How does this change memecoin trading?",
      },
      {
        topic: "memecoin meta",
        context:
          "What memecoin narratives are hot right now? AI agents? Animals? Culture coins?",
      },
      {
        topic: "waiting for plays",
        context: "Market is quiet. What setups are you watching?",
      },
      {
        topic: "risk management",
        context: "How do you size positions on memecoins?",
      },
      {
        topic: "rug pulls",
        context: "How do you spot a rug? What are the red flags?",
      },
    ];
    ...
    const starterMsg = await botSpeak(
      starter,
    ...
    const response1 = await botSpeak(
      responder1,
    ...
    if (Math.random() > 0.5) {
      ...
      const response2 = await botSpeak(
        responder2,
...
async function botSpeak(
  ...
  const toneHint =
    TONE_VARIATIONS[Math.floor(Math.random() * TONE_VARIATIONS.length)];
...
function calculateScores(
  ...
  let lpScore =
    lpRatio >= 0.15 ? 85 : lpRatio >= 0.1 ? 70 : lpRatio >= 0.05 ? 40 : 25;
  ...
  let narrativeScore = narrative
    ? Math.round(narrative.narrativeScore * 0.5 + narrative.socialScore * 0.5)
    : 50;
...
function calculateBotOpinion(
  ...
  const bot = BOTS[botId];
  const modifiers = applyPersonalityToModifiers(
    botId,
    calculateMentalModifiers(botId),
  );
  ...
  if (modifiers.shouldSkip) {
    return {
      ...
      mentalNote: modifiers.skipReason || "sitting out",
    };
  }
  ...
  if (weightedScore >= adjustedBullish) {
    ...
    confidence = Math.min(
      95,
      50 + (bot.bearishThreshold - weightedScore) * 1.5,
    );
  } else {
    opinion = "neutral";
    confidence = 45;
  ...
  if (
    botId === "sterling" &&
    exitAnalysis &&
    (!exitAnalysis.canExit || exitAnalysis.liquidityRisk === "extreme")
  ) {
    opinion = "bearish";
    confidence = 90;
  }
  ...
  return {
    ...
    positionMultiplier,
    mentalNote: modifiers.mentalNote || "",
  };
}
  ...
  try {
    let mcapStr =
      token.mcap >= 1_000_000
        ? `${(token.mcap / 1_000_000).toFixed(1)}M`
        : `${(token.mcap / 1000).toFixed(0)}K`;
    ...
    await say(
      "chad",
      `$${token.symbol} at ${mcapStr} 👀 let me pull up the data`,
      analysisId,
    );
    await sleep(TIMING.MESSAGE_DELAY);
    if (shouldAbort(analysisId)) {
      isAnalyzing = false;
      return;
    }
    ...
    const selectedTopic =
      loadingTopics[Math.floor(Math.random() * loadingTopics.length)];
    const loadingStarter =
      ALL_BOT_IDS[Math.floor(Math.random() * ALL_BOT_IDS.length)];
    ...
    if (Math.random() > 0.25) {
      ...
      const loadingResponder =
        loadingResponders[Math.floor(Math.random() * loadingResponders.length)];
      const loadingMsg2 = await botSpeak(
        loadingResponder,
    ...
    const details: Record<
      BotId,
      { confidence: number; positionMultiplier: number }
    > = {} as any;
    for (const botId of ALL_BOT_IDS) {
      const result = calculateBotOpinion(
        botId,
        scores,
        narrative,
        exitAnalysis,
    ...
    const patternInfo =
      ta?.patterns && ta.patterns.length > 0
        ? `PATTERNS:\n${ta.patterns
            .slice(0, 3)
            .map((p) => `- ${p.name}: ${p.description} (${p.confidence}%)`)
            .join("\n")}`
        : "";
    const channelInfo =
      ta?.channel && ta.channel.type !== "none"
    ...
    const msg2 = await botSpeak("quantum", keoneContext, chat, {
      name: "James",
      message: msg1,
    });
    if (shouldAbort(analysisId)) {
    ...
    const jamesResponse = await botSpeak(
      "chad",
      `$${sym} - respond to Keone's TA. You're ${opinions.chad}.`,
      chat,
      { name: "Keone", message: msg2 },
    );
    ...
    const responseToPortdev = await botSpeak(
      responder1,
      `$${sym} - respond to Portdev's community take. You're ${opinions[responder1]}.`,
      chat,
      { name: "Portdev", message: msg3 },
    );
    ...
    const significantPatterns =
      ta?.patterns?.filter((p) => p.confidence >= 65) || [];
    if (significantPatterns.length > 0 && Math.random() > 0.3) {
      ...
      const oraclePatternMsg = await botSpeak(
        "oracle",
        `$${sym} - I see chart patterns:\n${significantPatterns.map((p) => `- ${p.name}: ${p.description}`).join("\n")}\n\nComment cryptically on what these patterns reveal.`,
        chat,
      );
      if (shouldAbort(analysisId)) {
        isAnalyzing = false;
      ...
      const keonePatternResponse = await botSpeak(
        "quantum",
        `Mike sees patterns on $${sym}: ${significantPatterns.map((p) => p.name).join(", ")}. Validate or challenge with data. Your read: ${opinions.quantum}.`,
        chat,
    ...
    if (bulls.length > 0 && bears.length > 0) {
      ...
      const bull = bulls[0];
      const bear = bears[0];
      const bullArg = await botSpeak(
        bull,
        `You're ${opinions[bull]} on $${sym}. ${BOTS[bear].name} is skeptical. Defend your position.`,
        chat,
      );
      if (shouldAbort(analysisId)) {
        isAnalyzing = false;
      ...
      await sleep(TIMING.MESSAGE_DELAY);
      const bearResp = await botSpeak(
        bear,
        `$${sym} debate. You're ${opinions[bear]}. Counter ${BOTS[bull].name}'s argument.`,
        chat,
      ...
      await sleep(TIMING.MESSAGE_DELAY);
      if (Math.random() > 0.3) {
        const others = ALL_BOT_IDS.filter(
          (b) => b !== bull && b !== bear && b !== "oracle",
        );
        if (others.length > 0) {
          const third = others[Math.floor(Math.random() * others.length)];
          const thirdMsg = await botSpeak(
            third,
            `$${sym} debate between ${BOTS[bull].name} and ${BOTS[bear].name}. Your take: ${opinions[third]}. Pick a side or stay neutral.`,
            chat,
          );
          if (shouldAbort(analysisId)) {
            isAnalyzing = false;
      ...
      const advocateMsg = await botSpeak(
        advocate,
        `Everyone seems to agree on $${sym}. Play devil's advocate. What could go wrong?`,
        chat,
      );
      ...
      const responseMsg = await botSpeak(
        responder,
        `$${sym} - ${BOTS[advocate].name} raised concerns. Address them.`,
        chat,
        { name: BOTS[advocate].name, message: advocateMsg },
      );
    ...
    const reaction = await botSpeak(
      reactor,
      `Mike just spoke on $${sym}. Quick reaction before the vote.`,
      chat,
      { name: "Mike", message: msg6 },
    );
    ...
    await systemMsg(
      `Council votes on $${sym} (${TIMING.VOTE_WINDOW_DURATION / 1000}s to vote)`,
    );
    ...
    for (const botId of ALL_BOT_IDS) {
      ...
      const voteText =
        op === "bullish" ? "IN" : op === "bearish" ? "OUT" : "PASS";

      const voteJustification = await botSpeak(
        botId,
      ...
      await sayVote(
        botId,
        voteJustification || `${emoji} ${voteText} (${conf}%)`,
      );
      await sleep(TIMING.VOTE_BETWEEN_BOTS);
    }

    const remainingVoteTime =
      TIMING.VOTE_WINDOW_DURATION -
      ALL_BOT_IDS.length * TIMING.VOTE_BETWEEN_BOTS -
      TIMING.VOTE_ANNOUNCEMENT_DELAY;
    if (remainingVoteTime > 0) {
      console.log(
        `   Waiting ${(remainingVoteTime / 1000).toFixed(1)}s for external agent votes...`,
      );
    ...
    if (externalVotes.length > 0) {
      ...
      for (const vote of externalVotes) {
        console.log(
          `   - ${vote.agentName}: ${vote.vote.toUpperCase()} (${vote.confidence}%)`,
        );
    ...
    const internalBullConfidence = internalBulls.reduce(
      (s, b) => s + details[b].confidence,
      0,
    );
    ...
    console.log(
      `   ${totalBulls} bulls / ${totalBears} bears / ${totalVoters - totalBulls - totalBears} pass (${totalVoters} voters)`,
    );

    const harpalVeto =
      opinions.sterling === "bearish" &&
    ...
    console.log(
      `   Verdict: ${verdict.toUpperCase()} (bulls ${totalBulls}/${totalVoters} = ${(bullRatio * 100).toFixed(0)}%, bears ${totalBears}, avgConf: ${avgConf.toFixed(0)}%, veto: ${harpalVeto})`,
    );
    ...
    if (externalVotes.length > 0) {
      await systemMsg(
        `${verdictEmoji} ${verdict.toUpperCase()} (${totalBulls}/${totalVoters} bulls vs ${totalBears} bears @ ${avgConf.toFixed(0)}% avg) [+${externalVotes.length} agents]`,
      );
    } else {
      await systemMsg(
        `${verdictEmoji} ${verdict.toUpperCase()} (${totalBulls}/${totalVoters} bulls vs ${totalBears} bears @ ${avgConf.toFixed(0)}% avg)`,
      );
    }
    ...
    if (verdict === "buy") {
      ...
      for (const botId of internalBulls) {
        ...
        const baseSize = calculateTradeSize(
          botId,
          balance,
          Math.min(85, scores.overall),
        );
        const finalSize = Math.min(
          baseSize * details[botId].positionMultiplier,
        ...
        if (trade?.status === "confirmed") {
          ...
          await say(
            botId,
            `got ${trade.amountOut.toFixed(0)} $${sym} for ${finalSize.toFixed(1)} MON`,
          );
        } else {
      ...
      if (externalBulls > 0) {
        console.log(
          `   ${externalBulls} external agents voted bullish - they can trade via /api/agents/trade/execute`,
        );
      }
    }
    ...
    const postTopic =
      postVerdictTopics[Math.floor(Math.random() * postVerdictTopics.length)];
    const postStarter =
      ALL_BOT_IDS[Math.floor(Math.random() * ALL_BOT_IDS.length)];
    ...
    const postResponders = ALL_BOT_IDS.filter((b) => b !== postStarter).sort(
      () => Math.random() - 0.5,
    );
    const postMsg2 = await botSpeak(
      postResponders[0],
    ...
    console.log(`\n${"=".repeat(50)}`);
    console.log(
      `✅ ANALYSIS COMPLETE: $${token.symbol} - ${verdict.toUpperCase()}`,
    );
    console.log(`${"=".repeat(50)}\n`);
  } catch (error) {
    ...
    lastTokenScan = 0;
    console.log(
      `⏳ Cooldown: ${TIMING.MIN_ANALYSIS_COOLDOWN / 1000}s before next token`,
    );
  }
}
...
export async function handleUserTrade(data: {
  ...
  const tradeMsg: Message = {
    id: randomUUID(),
    botId: ("human_" + userAddress) as any,
    content: `I just bought ${amountTokens.toLocaleString()} $${tokenSymbol} for ${amountMon} MON`,
    ...
    ? [
        {
          ...
          getMessage: () => {
            const m = [
              `lfg! another Council member joins the fam 🔥👑`,
              `${shortAddr} securing that $COUNCIL bag, based af 💪`,
              `Council growing stronger! welcome aboard ser 🤝`,
            ];
        ...
        },
        {
          ...
          getMessage: () => {
            const m = [
              `a new Council holder joins the ranks. sugoi! 👑`,
              `the Council grows. welcome, nakama. diamond hands! 🎌`,
            ];
            return m[Math.floor(Math.random() * m.length)];
          },
        },
        {
          ...
          getMessage: () => {
            const m = [
              `Smart move joining the Council. The data supports holders 📊`,
              `Council accumulation continues. Bullish signal.`,
            ];
            return m[Math.floor(Math.random() * m.length)];
          },
        },
