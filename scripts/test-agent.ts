...
async function main() {
  ...
  console.log(
    `🔑 Private Key: ${AGENT_PRIVATE_KEY ? "✅ provided" : "❌ not set (trade tests skipped)"}`,
  );
  console.log(
    `📋 Existing API Key: ${EXISTING_API_KEY ? "✅ reusing" : "❌ will register new"}`,
  );
  console.log("");
  ...
  try {
    ...
    fail(
      "API reachable",
      `${e.message} — is the server running on ${API_URL}?`,
    );
    printSummary();
    return;
  }
  ...
  if (!apiKey) {
    ...
    const { status: s1 } = await request("POST", "/api/agents/register", {
      name: "A",
    });
    ...
    // Test: duplicate name
    const { status: s3 } = await request("POST", "/api/agents/register", {
      name,
    });
  ...
  const { status: badAuth } = await request(
    "GET",
    "/api/agents/me",
    undefined,
    authed("council_invalid_key"),
  );
  if (badAuth === 401) pass("Reject invalid API key");
  else fail("Reject invalid API key", `expected 401 got ${badAuth}`);

  // Test: valid API key
  const { status: goodAuth, data: meData } = await request(
    "GET",
    "/api/agents/me",
    undefined,
    authed(apiKey),
  );
  if (goodAuth === 200 && meData.agent?.id === agentId)
    pass("Authenticate with valid key");
  ...
  // Test: list agents
  const { status: listStatus, data: listData } = await request(
    "GET",
    "/api/agents",
  );
  ...
  // Test: leaderboard
  const { status: lbStatus, data: lbData } = await request(
    "GET",
    "/api/agents/leaderboard",
  );
  if (lbStatus === 200)
    pass("Leaderboard", `${lbData.leaderboard?.length || 0} entries`);
  else fail("Leaderboard", `status ${lbStatus}`);

  // Test: vote status (public)
  const { status: vsStatus, data: vsData } = await request(
    "GET",
    "/api/agents/vote-status",
  );
  if (vsStatus === 200 && "isOpen" in vsData)
    pass("Vote status", `open: ${vsData.isOpen}`);
  ...
  if (ctxStatus === 200) {
    ...
    pass(
      "Get context",
      token
        ? `$${token.symbol} @ $${token.mcap?.toLocaleString()}`
        : "no active token",
    );
  } else {
  ...
  // Test: empty content
  const { status: emptySpeak } = await request(
    "POST",
    "/api/agents/speak",
    { content: "" },
    authed(apiKey),
  ...
  const { status: longSpeak } = await request(
    "POST",
    "/api/agents/speak",
    { content: longContent },
    authed(apiKey),
  ...
  if (firstSpeak === 200 && firstSpeakData.success) {
    pass(
      "Send first message",
      `triggeredResponses: ${firstSpeakData.triggeredResponses}`,
    );
  } else {
  ...
  if (currentToken) {
    const { status: tokenSpeak } = await request(
      ...
      "/api/agents/speak",
      {
        content: `What do you think about $${currentToken.symbol}? James, you aping?`,
  ...
  if (noAddrVote === 400) pass("Reject vote without tokenAddress");
    ...
    fail("Reject vote without tokenAddress", `expected 400 got ${noAddrVote}`);
  ...
  const { data: liveVoteStatus } = await request(
    "GET",
    "/api/agents/vote-status",
    undefined,
    authed(apiKey),
  ...
  if (liveVoteStatus.isOpen && liveVoteStatus.tokenAddress) {
    const { status: voteOk, data: voteData } = await request(
      ...
      "/api/agents/vote",
      {
        tokenAddress: liveVoteStatus.tokenAddress,
        vote: "bullish",
        confidence: 72,
      },
      authed(apiKey),
    );
    if (voteOk === 200 && voteData.success)
      pass("Submit vote on open window", `bullish 72%`);
    else
      fail(
        "Submit vote on open window",
        `status ${voteOk}: ${JSON.stringify(voteData)}`,
      );
  } else {
  ...
  if (closedVote === 400) pass("Reject vote on closed/wrong window");
    ...
    fail(
      "Reject vote on closed/wrong window",
      `expected 400 got ${closedVote}`,
    );
  ...
  const { status: infoStatus, data: infoData } = await request(
    "GET",
    "/api/agents/council/info",
  );
  if (
    infoStatus === 200 &&
    infoData.tokenAddress &&
    infoData.symbol === "COUNCIL"
  ) {
    pass(
      "Get $COUNCIL token info",
      `${infoData.tokenAddress.slice(0, 10)}... on ${infoData.chain}`,
    );
  ...
  if (csStatus === 200 && "holdsCouncil" in csData) {
    pass(
      "Get council status",
      `holds: ${csData.holdsCouncil}, features: ${JSON.stringify(csData.features)}`,
    );
  } else {
  ...
  if (!AGENT_PRIVATE_KEY) {
    ...
    if (buyStatus === 200 && buyData.success) {
      pass(
        "Buy $COUNCIL token",
        `tx: ${buyData.txHash?.slice(0, 16)}... got ${buyData.amountOut}`,
      );
    } else {
      fail(
        "Buy $COUNCIL token",
        `status ${buyStatus}: ${buyData.error || JSON.stringify(buyData)}`,
      );
    }

    // Verify balance increased
    await sleep(2000);
    const { data: newBal } = await request(
      "GET",
      "/api/agents/council/balance",
      undefined,
      authed(apiKey),
    ...
    const { data: newCs } = await request(
      "GET",
      "/api/agents/council-status",
      undefined,
      authed(apiKey),
  ...
  if (!AGENT_PRIVATE_KEY) {
    ...
    if (reqStatus === 200 && reqData.success) {
      ...
      pass(
        "Request analysis — handled gracefully",
        `${reqStatus}: ${reqData.error || reqData.message}`,
      );
    }
  }
  ...
  const { status: predStatus, data: predData } = await request(
    "GET",
    "/api/agents/predictions",
  );
  if (predStatus === 200 && Array.isArray(predData.predictions)) {
    ...
    for (const p of predData.predictions.slice(0, 3)) {
      const active =
        !p.resolved && !p.cancelled && p.endTime * 1000 > Date.now();
      console.log(
        `      #${p.id}: "${p.question}" — ${active ? "🟢 active" : "⚫ ended"} — pool: ${p.prizePool} MON`,
      );
    }
  } else {
    // Predictions might fail if contract not deployed — that's ok
    pass(
      "List predictions — handled gracefully",
      `${predStatus}: ${predData.error || "ok"}`,
    );
  }
  ...
  if (!AGENT_PRIVATE_KEY) {
    ...
    if (activePrediction) {
      console.log(
        `   🎲 Placing bet on prediction #${activePrediction.id}: "${activePrediction.question}"`,
      );
      ...
      if (betStatus === 200 && betResult.success) {
        pass(
          ...
          `tx: ${betResult.txHash?.slice(0, 16)}...`,
        );
        ...
        pass(
          "Place bet — handled gracefully",
          `${betStatus}: ${betResult.error || "ok"}`,
        );
      }
    } else {
  ...
  if (!AGENT_PRIVATE_KEY || !currentToken) {
    ...
    console.log(
      `   🔄 Trading ${TRADE_AMOUNT_MON} MON → $${currentToken.symbol}...`,
    );
    ...
    if (tradeStatus === 200 && tradeResult.success) {
      pass(
        "Execute trade",
        `tx: ${tradeResult.txHash?.slice(0, 16)}... got ${tradeResult.amountOut}`,
      );
    } else {
      fail(
        "Execute trade",
        `${tradeStatus}: ${tradeResult.error || JSON.stringify(tradeResult)}`,
      );
    }
  }
  ...
  const { status: finalHistStatus, data: finalHist } = await request(
    "GET",
    "/api/agents/history?limit=20",
    undefined,
    authed(apiKey),
  );
  if (finalHistStatus === 200) {
    const ourMessages =
      finalHist.messages?.filter((m: any) => m.botId?.includes(agentId)) || [];
    const botResponses =
      finalHist.messages?.filter(
        (m: any) =>
          !m.botId?.includes(agentId) &&
          !m.botId?.startsWith("system") &&
          !m.botId?.startsWith("agent_"),
      ) || [];
    pass(
      "Final history check",
      `${ourMessages.length} our msgs, ${botResponses.length} bot responses`,
    );
    ...
    for (const m of last) {
      ...
      const prefix = isOurs
        ...
        : m.botId?.startsWith("agent_")
          ...
          : `💬 ${m.botId}`;
      console.log(
        `      ${prefix}: ${m.content?.slice(0, 65)}${m.content?.length > 65 ? "..." : ""}`,
      );
    }
  } else {
  ...
  if (AGENT_PRIVATE_KEY) {
    console.log(
      `   API_KEY=${apiKey} AGENT_PK=${AGENT_PRIVATE_KEY} npx ts-node test-agent-full.ts`,
    );
  }
  console.log("");
}
...
function printSummary() {
  ...
  console.log("╠══════════════════════════════════════════════════╣");
  console.log(
    `║  ✅ Passed:  ${String(passed).padEnd(4)} / ${total}                          ║`,
  );
  console.log(
    `║  ❌ Failed:  ${String(failed).padEnd(4)} / ${total}                          ║`,
  );
  console.log(
    `║  ⏭️  Skipped: ${String(skipped).padEnd(4)} / ${total}                          ║`,
  );
  console.log("╚══════════════════════════════════════════════════╝");
